...
const WaveformVisualizer = (): JSX.Element => {
    ...
    const trueSignalPoints = useMemo(() => {
        ...
        for (let i = 0; i <= DURATION * resolution; i++) {
            ...
            const y =
                CENTER_Y - AMPLITUDE * Math.sin(2 * Math.PI * frequency * t);
    ...
    return (
        <div style={styles.container}>
            ...
            <div style={styles.controlGroup}>
                <div style={styles.label}>
                    ...
                    <span style={{ color: isAliasing ? "#ff4444" : "#4caf50" }}>
                        {samplingRate} Hz{" "}
                        {isAliasing ? "(ALIASING WARNING)" : "(OK)"}
                    </span>
                </div>
                <input
                    type="range"
                    ...
                    value={samplingRate}
                    onChange={(e) =>
                        setSamplingRate(parseFloat(e.target.value))
                    }
                    style={styles.slider}
                />
            </div>
            ...
            <div style={styles.legend}>
                <div style={styles.legendItem("#555")}>
                    ── Real Signal (Truth)
                </div>
                <div style={styles.legendItem("#ff4444")}>
                    ●--● Sampled Signal (Scope)
                </div>
            </div>
        </div>
    );
